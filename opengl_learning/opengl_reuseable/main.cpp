...
int main() {
	...
	GLfloat vertices[] = {
			-0.5f,-0.5f* float((sqrt(3))) / 3, 0.0f, 0.5f,0.4f,0.3f		,//0 //max left
			0.5f, -0.5f* float((sqrt(3))) / 3, 0.0f, 0.5f,0.4f,0.3f		,//1 max right 
			0.0f, 0.5f * float((sqrt(3))) / 3, 0.0f, 0.8f,0.4f,0.3f			,//2 upper mid
			-0.25f,0.0f*  float(sqrt(3)) /  3, 0.0f, 1.0f,0.5f,0.5f			, //3 left upper
			0.0f, -0.5f* float((sqrt(3))) /3,  0.0f, 1.0f,0.5f,0.5f			,//middle 4
			0.25f,0.0f*  float(sqrt(3))     /3,0.0f, 1.0f,0.5f,0.5f			,
	...
	VAO_1.linkAttrib(VBO_1, 1, 3,GL_FLOAT,6*sizeof(float),(void*)(3*sizeof(float)));
	VAO_1.unbind();
	EBO_1.unbind();
	VBO_1.unbind();
